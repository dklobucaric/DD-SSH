...
void MainWindow::addWelcomeTab()
{
    ...
    welcome->setPlainText(
        "DD-SSH — Andromeda\n\n"
        "A clean cross-platform SSH client, session manager, and tester-ready SFTP File Manager alpha.\n\n"
        ...
        "- portable plaintext secrets in dd-ssh.json for early alpha portability\n"
        "- password and private-key authentication\n"
        "- known_hosts trust handling with multi-key host entries per host:port\n"
        "- create/edit/delete saved sessions and duplicate-session warnings\n"
        "- xterm.js terminal renderer with bundled local assets\n"
        "- SSH PTY resize sync and terminal lifecycle handling\n"
        "- tested terminal apps: htop, nano, vim, top, clear\n"
        "- multiline terminal paste fixed for nano/YAML/config editing\n"
        ...
        "- Retry selected for Done/Failed/Cancelled/Skipped queue items\n"
        "- Overwrite, Skip, Overwrite all, and Skip all decisions\n"
        "- overwrite metadata dialogs with existing/incoming size and modified time\n"
        "- local and remote queued delete for regular files, symlinks, and empty folders\n"
        "- destructive delete confirmations shown when the queue reaches the delete item\n"
        "- queue completion summaries and local/remote refresh after queue runs\n"
        "- SFTP/file-transfer diagnostic logs when logging is enabled\n\n"
        "Known alpha limits:\n"
        "- no sync/mirror engine\n"
        "- no resume for partial transfers\n"
        ...
        "- 0.1.x — Andromeda / current terminal + File Manager alpha line\n"
        "- 0.2.x — Orion\n"
        "- 0.3.x — Vega\n"
        "- 0.4.x — Cassiopeia\n"
        "- 1.0.x — Apollo\n\n"
