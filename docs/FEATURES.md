...
- Saved-session context menu includes `Open File Manager`
...
- `Download selected now` downloads one selected remote file into the currently open local folder
- `Queue upload` is the preferred local upload workflow; the older immediate single-file upload helper is still kept internally but is no longer exposed as the main local toolbar button
